...
def convert_start_transaction_response_json(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return input_df.withColumn("new_body", from_json(input_df.body, start_transaction_response_body_schema()))
    ###
...
def convert_start_transaction_request_json(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return input_df.withColumn("new_body", from_json(input_df.body, start_transaction_request_body_schema()))
    ###
...
def join_with_start_transaction_request(input_df: DataFrame, join_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return input_df.join(join_df, on=input_df.message_id == join_df.message_id, how= "inner") \
                   .select(input_df.charge_point_id.alias("charge_point_id"),input_df.new_body.transaction_id.alias("transaction_id"), 
                           join_df.new_body.meter_start.alias("meter_start"), join_df.new_body.timestamp.alias("start_timestamp"))
    ###
...
def join_stop_with_start(input_df: DataFrame, join_df: DataFrame) -> DataFrame:
    ...
    ### YOUR CODE HERE
    input_df = input_df.join(join_df, on=input_df.new_body.transaction_id == join_df.transaction_id, how="left") \
                   .select(join_df.charge_point_id, input_df.new_body.transaction_id.alias("transaction_id"),
                           join_df.meter_start, input_df.new_body.meter_stop.alias("meter_stop"),
                           join_df.start_timestamp, input_df.new_body.timestamp.alias("stop_timestamp"))
    return input_df
    ###
...
def convert_start_stop_timestamp_to_timestamp_type(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.withColumn("start_timestamp", to_timestamp("start_timestamp")) \
...
def calculate_total_time_hours(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.withColumn("total_time", 
                                   round((input_df.stop_timestamp.cast(LongType()) - input_df.start_timestamp.cast(LongType()))/3600, 2))
    return input_df
    ###
...
def calculate_total_energy(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.withColumn("total_energy", round(input_df.meter_stop - input_df.meter_start, 2).cast(DoubleType()))
    return input_df
    ###
...
#  |-- new_body: struct (nullable = true)
#  |    |-- connector_id: integer (nullable = true)
#  |    |-- transaction_id: integer (nullable = true)
#  |    |-- meter_value: array (nullable = true)
#  |    |    |-- element: struct (containsNull = true)
#  |    |    |    |-- timestamp: string (nullable = true)
#  |    |    |    |-- sampled_value: array (nullable = true)
#  |    |    |    |    |-- element: struct (containsNull = true)
#  |    |    |    |    |    |-- value: string (nullable = true)
#  |    |    |    |    |    |-- context: string (nullable = true)
#  |    |    |    |    |    |-- format: string (nullable = true)
#  |    |    |    |    |    |-- measurand: string (nullable = true)
...
def reshape_meter_values(input_df: DataFrame) -> DataFrame:
    ...
    ### YOUR CODE HERE
    return input_df.select("*", explode("new_body.meter_value").alias("meter_value")).\
                        ...
                        withColumn("value", col("sampled_value.value").cast(DoubleType())). \
...
def join_with_target_df(input_df: DataFrame, joined_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return input_df.join(joined_df, on= [input_df.transaction_id == joined_df.transaction_id], how="left").\
                    select(input_df.charge_point_id,
...
def cleanup_columns(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return input_df.select(input_df.charge_point_id,
                           input_df.transaction_id,
                           input_df.start_timestamp,
                           input_df.stop_timestamp,
                           input_df.total_time,
                           input_df.total_energy,
                           input_df.total_parking_time)
    ###
