...
def convert_to_timestamp(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return input_df.withColumn("converted_timestamp", to_timestamp("write_timestamp"))
    ###
...
def most_recent_message_of_charge_point(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    return input_df. \
        withColumn("rn", row_number().over(Window.partitionBy("charge_point_id").orderBy(col("converted_timestamp").desc()))). \
        where(col("rn") == 1)
    ###
