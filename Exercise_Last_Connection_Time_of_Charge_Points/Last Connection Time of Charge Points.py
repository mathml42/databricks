...
def convert_to_timestamp(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.withColumn("converted_timestamp", to_timestamp("write_timestamp"))
    return input_df
    ###
...
def most_recent_message_of_charge_point(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    meassage_window = Window.partitionBy("charge_point_id").orderBy(col('converted_timestamp').desc())
    input_df = input_df.withColumn("rn", row_number().over(meassage_window)) \
                .where(col('rn') == 1) 
    return input_df
    ###
...
def cleanup(input_df: DataFrame) -> DataFrame:
    ### YOUR CODE HERE
    input_df = input_df.drop("rn","write_timestamp")
    return input_df
    ###
